...
papers: [safetynets, zkcnn, zkllm, zkgpt, zkpytorch, deepprove, spagkr, zkml-kang, ezkl, artemis, jolt-atlas, mystique, lu-et-al, zktorch, zen, vcnn, zip, hao-et-al, nanozk]
status: draft
---
...
:::quote{src="zkGPT" sec="§5, Constraint fusion"}
...
proofs are kilobytes and verification is milliseconds, which is what makes on-chain
settlement plausible at all. GKR's megabyte proofs are not going on a blockchain.

**What it is bad at.** The prover. Every non-linearity becomes constraints, the constraint
count explodes, and [[zkml-kang]] is consequently the slowest LLM entry in the table — the
baseline the GKR systems beat by orders of magnitude. Key generation is its own tax
...
**What it is bad at.** We do not honestly know, and that is the finding.
[[zktorch]]'s own per-model table has not been extracted into `papers.yml`; both LLM rows
there come from *secondhand* sources that disagree with each other, one of which is a blog
post written by a competitor's author.
...
the prover to fit the model, proving native IEEE-754 double-precision inference with
piecewise-polynomial lookup approximations for the activations, plus extra arithmetic
constraints hardening those lookups against a malicious prover. It is not an LLM system and
does not belong on the throughput chart. It belongs here because it is the standing
disproof of the assumption everything else makes.
...
non-ZK configuration and its competitor's is not, that is a fifth confounder on top of
[the four we already have](./what-is-proven/).
:::
